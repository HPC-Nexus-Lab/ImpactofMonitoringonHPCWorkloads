...
@rfm.simple_test
class dt_test(rfm.RegressionTest):
    problem_size = variable(str, value='A')
    graph = variable(str, value='BH')
    valid_systems = ['*']
    valid_prog_environs = ['*']
    ...
    @run_before('compile')
    def set_class(self):
        self.build_system.options = [ "dt", f"CLASS={self.problem_size}" ]
    ...
    def set_executable(self):
        self.executable = f"bin/dt.{self.problem_size}.x {self.graph}"
