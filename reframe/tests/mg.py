...
@rfm.simple_test
class mg_test(rfm.RegressionTest):
    problem_size = variable(str, value='A')
    valid_systems = ['*']
    valid_prog_environs = ['*']
    parallel_launcher = 'mpirun'
    r_num_tasks = variable(int, value=32)
    num_cpus_per_task = 1
    ...
    @run_before('compile')
    def set_class(self):
        self.build_system.options = [ "mg", f"CLASS={self.problem_size}" ]
    ...
    def set_executable(self):
        self.executable = f"bin/mg.{self.problem_size}.x"
